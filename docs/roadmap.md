...
- **Phase 4c baseline: the scalar-believing stack against the 4b field —
  ...
  **What this sets up for 4c.** The bar is not open-loop's ~20-26mm; it is
  ...
  needs care.
- **Phase 4c step 0: oracle-field headroom + capsule ablation — complete (the
  measurement that says 4c is worth building, before any learned model).**
  Before collecting or fitting anything, put the KNOWN field into the PLANNER
  (no learning) and ask whether even a perfect model beats the scalar baseline.
  If the oracle cannot win, the scenarios or metrics are the problem and no
  amount of learning fixes that. Scripts only (`scripts/four_c_oracle_headroom.py`);
  the planner change is a `NeedleParams` construction — KinodynamicRRT is
  field-native (verified nothing in its path reads `params.kappa` as a scalar),
  filters stay scalar, and the field is served through a precomputed LOOKUP
  TABLE (max abs error vs `kappa_at` 5.9e-7, the plumbing 4c reuses). The
  headline metric is OPEN-LOOP endpoint error, because both closed loops are
  CENSORED at `goal_tolerance=3.0` (every success lands in [0,3) and reports
  ~3.0), so measuring the field's value on closed-loop endpoint risks a FALSE
  NEGATIVE — a correct model that looks worthless against a ruler that bottoms
  out. The baseline condition reproduces the committed field-world five-way
  exactly (open-loop and fixed), confirming like-for-like.

  **There is large headroom, and the win is fidelity/tracking, not a different
  plan.** Open-loop endpoint (medians over 5 seeds): the oracle cuts it from
  20.0 → 2.9mm (`constrained`) and 26.5 → 2.9mm (`open`). The baseline is
  UNCENSORED at 20-26mm, so the ~17-24mm gap is real, not a floor artifact.
  On the non-saturated FEASIBILITY metric the difference is categorical: the
  baseline COLLIDES with the wall on 3/5 `constrained` seeds and leaves the
  workspace on 2/5 `open` seeds; the oracle does neither, ever. Even the
  censored closed loop leaks signal — the oracle needs 0 replans in every run
  vs 8-9 for the baseline, and reaches goal where the baseline falls short. The
  confound (a field-believing planner PLANS differently, not just tracks better)
  resolves cleanly toward tracking: the plans are near-identical in geometry
  (lengths within a few mm, node counts within one) but plan-to-execution
  deviation is 0.1-0.2mm for the oracle vs 4-13mm mean / up to 36mm for the
  baseline. So a perfect model makes open-loop execution as accurate as
  closed-loop, which is exactly the value a correct model should deliver.

  **The capsule ablation: the capsule's share is large where it matters, and
  the split is mechanistic, not random.** Two ablated planner beliefs isolate
  it — C (true field EXCEPT the capsule, held at 1/29) and its mirror D (1/29
  EXCEPT the true capsule). The capsule is 56% of the open-loop win on
  `constrained` (C−A = 9.6mm of the 17.1mm win, and blanking it REINTRODUCES a
  wall collision the full oracle avoided, 0/5 → 1/5) but only 13% on `open`
  (3.0mm, no feasibility impact). The cause is geometric: `constrained`'s
  capsule at y65-70 sits directly beneath the critical wall at y72-78, so
  capsule curvature sets the approach to the gap; `open`'s path runs up x≈30 far
  from the obstacle at x=75, so capsule error just adds to gross drift with
  nothing to punish it. So the answer to "does knowing the capsule is a small
  share make data collection easier?" is NO — it is not a small share where a
  structure sits past it. (Report per-scenario, never pooled: these would
  average to a misleading ~35%.)

  **The mirror earned its runtime — endpoint and collision DECOUPLE.** On
  `constrained`, D (capsule-only-correct) recovers 14 of the 17mm endpoint but
  still collides 3/5 (same as baseline), while C (everything but the capsule)
  has WORSE endpoint (12.6mm) but only 1/5 collisions. So the capsule owns
  TERMINAL ACCURACY (it is the last high-contrast layer before the goal) while
  the peri-wall muscle/fat own COLLISION AVOIDANCE (they set the trajectory AT
  the wall); neither alone reaches zero collisions. And the interaction is real:
  (A−C)=9.6 and (D−B)=14.0 disagree and do not sum with their complements to the
  17.1mm oracle win, so the capsule's contribution depends on whether the other
  layers are right — a single-direction ablation would have reported one number
  that looks like "the capsule's share" and isn't. Guardrail that follows:
  `constrained` was not DESIGNED to stress the capsule (the capsule merely
  happens to sit under its wall) yet the capsule is already load-bearing there,
  so the measured share is a LOWER bound — a deliberately capsule-relevant
  held-out scenario would look like `constrained` or worse. Regenerated by
  `scripts/four_c_oracle_headroom.py`.
- **Phase 4c step 1: two synthetic gates before any real collection —
  complete, and the answer is a QUALIFIED go with a specific caveat found by
  the variance check.** Before collecting or fitting anything, two gates use the
  KNOWN field as ground truth (`scripts/four_c_gates.py`) to decide whether 4c
  CAN work: is the information present to be learned (identifiability), and does
  the realistic measurement rate resolve the crux capsule (observability). The
  inversion is Source C — fit the field to raw positions, field latent, controls
  known, on constant-b characterization arcs (kappa_eff = kappa(y), no duty
  cycle to deconvolve) — with a GP prior (mean = the scalar 1/29, physics
  length-scale ~3mm NOT fitted, so an under-observed region reverts to the
  already-validated capsule-BLANKED fallback). The forward operator is the Task
  1 `step` rolled forward (reused, not reimplemented); MAP by Gauss-Newton
  (`scipy.least_squares` over stacked data-misfit + GP-prior residual), Laplace
  covariance (J^T J)^{-1}. Scripts only; nothing under
  planning/models/estimation/control.

  **THE MATCHED-MODEL CAVEAT, printed in the script's own output (not just this
  file).** Both gates GENERATE with `TissueField` and INVERT with a model whose
  prior is tuned to that same field (smooth, layered, 1-D in depth, ~2mm
  transitions ~ the GP length-scale) — the classic inverse crime. So a GO
  certifies the INFORMATION IS PRESENT (identifiability/observability are
  forward-map properties, legitimately answered in the matched setting), NOT
  that 4c works on real tissue. Every verdict is labelled "information present
  under a MATCHED MODEL" so a favourable number cannot be lifted out of context
  later — the same failure the metric-saturation catch was.

  **Gate 1 (identifiability): the path-set determines everything, and only the
  strong levers separate.** Verdict is on HELD-OUT-shape prediction RMS (not
  in-sample, which flexibility can game), with kappa(y) and kappa(s) fitted at
  MATCHED capacity (same knot spacing / length-scale in each coordinate):
  - `template` (near-parallel launches, small heading fan — what a treatment
    template permits): does NOT separate (held-out gap +0.65mm, ~the 1mm noise
    floor). This is the scope the claim would collapse to WITHOUT the
    characterization framing.
  - `wide_fan` (launches well beyond template angles, both bevel signs): SEPARATES
    (+5.25mm).
  - `revisit` (steep constant-b arcs that curve back through depths, so a depth
    recurs at two arc lengths within one insertion): SEPARATES strongly
    (+12.81mm). This confirms the ordering predicted in the design review:
    clinically-realistic entry/heading variation is too weak; the separation
    genuinely requires the control diversity only a CHARACTERIZATION protocol
    (phantom/ex-vivo, free controls) provides — which is exactly why that
    framing was adopted.

  **Gate 2 (capsule recovery at interval=20): the MEAN recovers but the
  VARIANCE does not — and that is the finding the calibration refinement was
  added to catch.** Phase-diverse pooling ON; path-set = the separating
  `revisit` arcs AUGMENTED with capsule-DWELL arcs (separation and dwell are
  DIFFERENT properties, reported separately: dwell was 1331mm total in the 5mm
  band across 120 arcs). Across N ∈ {5,10,20,40,80} the capsule MEAN recovers at
  every N (R 15.6–17.5 vs true 15.1 — mean ok), but the ±1σ BAND is never
  honestly calibrated: over-wide at low N (all-knot coverage 0.92 at N=5) and
  progressively OVERCONFIDENT as N grows (coverage collapses 0.92 → 0.48
  all-knots, 0.67 → 0.00 IN the capsule by N=80). So the pooled data DOES carry
  the capsule mean at the realistic rate — but the Laplace/Gauss-Newton
  posterior tightens its band as evidence accumulates while the residual model
  error does not shrink with it, so the band ends up excluding the truth exactly
  where the fail-safe needs it wide.

  **The rate-vs-inference disambiguation (the point of running interval=5).**
  Because the MEAN already resolved at interval=20, this is NOT an observability
  wall — and interval=5 confirms it: 4× the measurements give the same capsule
  mean (R 15.9) and the SAME broken band (capsule coverage 0.33). So the
  limitation is the LINEARISED INVERSION's overconfident posterior, not the
  measurement rate — raising the rate would not fix it, which is precisely why
  the rate must NOT be raised to "make this work". The revert-to-prior fail-safe
  cannot trust this posterior's variance as-is; step 2 must either use a
  posterior that stays honest under accumulation (e.g. sampling / a
  marginalised GP rather than a Laplace point-linearisation, or an inflated
  observation model that accounts for the un-modelled residual) or gate the
  fail-safe on a calibration check rather than the reported σ.

  Net: a QUALIFIED go. The information to learn a spatial kappa(y) IS present
  from characterization-grade paths at the realistic rate (gate 1 separates,
  gate 2's mean recovers), so 4c is worth building — but the uncertainty half
  it needs for the safe fail-over is not free from the obvious inversion, and
  that is now a named step-2 problem rather than a surprise in deployment.
- **Phase 4c Gate 3 (model-misspecification robustness): REQUIRED, not yet run —
  a named commitment before any claim that 4c TRANSFERS.** Gates 1 and 2
  establish information-present under a MATCHED model (inverse crime); they say
  nothing about what happens when the real field is not a clean smooth kappa(y)
  of the assumed shape. Before any transfer claim, generate the synthetic ground
  truth from a field the GP CANNOT represent exactly and re-run the recovery +
  variance-calibration checks. The specific misspecifications to test, recorded
  so this is a commitment and not a good intention:
  (a) a capsule transition SHARPER than the GP length-scale (~1mm vs the assumed
      ~2-3mm), so the smoothness prior is wrong exactly at the crux;
  (b) a small LATERAL (x) tilt in the field, violating the depth-only kappa(y)
      assumption;
  (c) a NON-SIGMOID bump shape (e.g. Gaussian) instead of stacked sigmoids, so
      the parametric family differs from the truth;
  (d) CORRELATED (non-white) measurement noise, violating the independent-noise
      likelihood the inversion assumes.
  A clean recovery under (a)-(d) is what would upgrade the gate-1/2 "information
  present under a matched model" to "recoverable under realistic
